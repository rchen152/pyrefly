...
Note that `baseline` is a **project-level setting** and cannot be overridden in [`sub-config`](/docs/configuration#sub-configs) sections. If you need different baseline files for different parts of your codebase, consider using separate Pyrefly configuration files.
