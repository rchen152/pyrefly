...
Let’s imagine a project with a dependency structure that looks like this. A is the file that we are editing, and B is another file that we have open, which is affected by edits to the types in A. As a user, what we want is for changes we make in B to immediately update A, with minimal lagging.
...
The image below shows the intersection of A’s reverse dependencies and B’s dependencies in purple. Once those modules are checked, we can show updated diagnostics for open files to the user, while continuing to check the remaining modules in the background.

![nodes diagram purple](./blog_imgs/a_b_nodes_4.png)

In order to provide a smooth IDE experience, Pyrefly runs language server operations (e.g. hover, autocomplete, go-to-def) on one thread and rechecks on another thread,  that way users can still use language server features while a recheck is happening in the background. Previously, the main language server thread would send updated diagnostics to the IDE, and only did so once it received a signal from the recheck thread that a recheck had finished. Now with streaming diagnostics in Pyyrefly v0.52.0, the recheck thread can emit the diagnostics to the frontend directly.

However, making this change means that there can be potential race conditions between diagnostics emitted from the main language server thread and the recheck thread. To prevent this, while a recheck is ongoing we do not allow the main language server thread to send diagnostics for files that might need diagnostic streaming. In other words, only one thread may send diagnostics for a given file at a given time.

## Results

Combined, these two optimizations enable Pyrefly to provide diagnostics 18x faster than before. Going back to the example from the introduction, that specific edge case which was previously taking ~3.6 seconds, now typically takes less than 200ms on an M4 Macbook Pro. For users, this means type errors appear instantly after saving a file, a noticeable improvement:
